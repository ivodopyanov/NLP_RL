...
class Base(Layer):
    ...
    def get_value_masks(self, cursors, stack_mask):
        ...
        return {'stack_prev': stack_mask_prev_value,
    ...
    def calc_reduced_value(self, values):
        ...
        h = K.concatenate([values['stack_current'][:, self.hidden_dim:], values['stack_prev'][:, self.hidden_dim:]], axis=1)
    ...
    def calc_action(self, values):
        ...
        s_input = K.concatenate([values['stack_current'][:, self.hidden_dim:],
    ...
    def calc_stack_after_reduce(self, stack, value_masks, reduced):
        ...
        stack_reduce_result = self.insert_tensor_at_mask(stack, value_masks['stack_current'], K.zeros((self.batch_size, 2*self.hidden_dim)))
        stack_reduce_result = self.insert_tensor_at_mask(stack_reduce_result, value_masks['stack_prev'], reduced)
